...
class TimeFrameConsolidation(models.Model):
    ...
    picking_customer_consolidation = fields.One2many('picking.customer.consolidation','time_frame_consolidation_id', string="Picking customer consolidations")
...
class PickingConsolidationLine(models.Model):
    ...
    picking_consolidation_id = fields.Many2one('picking.consolidation', string='Picking consolidation',ondelete='cascade')
...
class SupplierConsolidationLine(models.Model):
    ...
    supplier_consolidation_id = fields.Many2one('picking.supplier.consolidation', string='Picking supplier consolidation',ondelete='cascade')
...
class PickingCustomerConsolidation(models.Model):    
    ...
    time_frame_consolidation_id = fields.Many2one('time.frame.consolidation',string='Time frame consolidation', ondelete='cascade', readonly=True)
    time_frame_id = fields.Many2one(related='time_frame_consolidation_id.time_frame_id', readonly=True)
    delivery_address = fields.Many2one('res.partner', string="Delivery address", readonly=True)
    customer_consolidation_lines = fields.One2many('customer.consolidation.line','customer_consolidation_id', string='Pickings consolidation')
...
class CustomerConsolidationLine(models.Model):    
    ...
    customer_consolidation_id = fields.Many2one('picking.customer.consolidation', string='Picking customer consolidation',ondelete='cascade')
